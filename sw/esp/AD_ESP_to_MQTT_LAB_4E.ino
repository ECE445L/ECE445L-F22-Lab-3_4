...
// ----------------  VARIABLES    ---------------------------------         
//
char  eid[20]           = "jtm4565_add3338";        // TODO: Update this with you EID
...
char        mqtt_broker[20]     = "";     // broker.emqx.io
char        port[5]             = "";     // 1883
int         mqtt_port;
...
const char  *pub_mode   // TODO: Update more of these depending on your specifications
        = "/b2w/mode"; 
const char  *pub_hour           = "/b2w/hour";  
const char  *pub_min            = "/b2w/min"; 
const char  *pub_sec            = "/b2w/sec";
const char  *pub_adc            = "/b2w/adc";
  ...
  while (Serial.available() > 0)      // Wait for date from the TM4C
  {
    ...
    else if (inchar == '\n')
    {
       if (bufpos  > 0) 
       {
        ...
        snprintf(topic_publish, sizeof(topic_publish), "%s%s", eid, pub_adc);
        client.publish(topic_publish,   adc,  1); 
      }
